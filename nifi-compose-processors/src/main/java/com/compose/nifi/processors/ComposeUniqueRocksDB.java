...
public class ComposeUniqueRocksDB extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        String logicalKey = flowFile.getAttribute(logicalKeyName);
        if (logicalKey == null) {
            session.transfer(flowFile, REL_FAILURE);
            return;
        }
        ...
        try {
            byte[] key = logicalKey.getBytes(StandardCharsets.UTF_8);
            ...
            context.yield();
            session.rollback();
            getLogger().error("Failed to check if {} unique due to {}", new Object[]{logicalKey, e});
        }
    }
